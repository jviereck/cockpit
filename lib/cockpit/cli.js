...
Requisition.prototype = {
    ...
    /**
     * The object that holds the data in case the command line is used as
     * ArgumentsProvider.
     */
    _argsProviderData: undefined,

    setArgsProviderData: function(data) {
        this._argsProviderData = data;
    },

    getArgsProviderData: function() {
        return this._argsProviderData;
    },

    /**
     * The function used by the canon to request required arguments.
     */
    _argsProvider: undefined,

    setArgsProvider: function(argsProviderFunc) {
        this._argsProvider = argsProviderFunc;
    },

    getArgsProvider: function() {
        return this._argsProvider;
    },
     ...
     */
    exec: function() {
        // If some parameters are requested, check if the current typed value
        // in the command line matches what request typed value was.
        // E.g. Requested arguments for "find" -> typed = "find ". If the cli's
        // value is now "replace " the argument request doesn't match anymore.
        if (this._argsProviderData != null) {
            var typed = (this.input && this.input.typed) ?
                            this.input.typed :
                            this.toCanonicalString();

            // Does it still match?
            if (typed.indexOf(this._argsProviderData.typed) == 0) {
                var request = this._argsProviderData.request;
                request.args = this.getArgsObject();
                this._argsProviderData.callback();
                this._argsProviderData = null;
                return;
            }
            // Nope.
            else {
