...
function CliView(cli, env) {
    ...
    this.createElements();
    this.createArgsProvider();
    this.update();
}
CliView.prototype = {
    createArgsProvider: function() {
        var cli = this.cli;
        function CliArgsProvider(request, callback) {
            var types = request.typed;
            if (types == null) {
                typed = [ request.command.name ]

                var args  = request.args,
                    params = request.command.params;

                // Loop over the parameters and stop once there is a parameter
                // that doesn't have an argument yet.
        ...
        cli.setArgsProvider(CliArgsProvider.bind(this));
    },
